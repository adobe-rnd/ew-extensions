...
import { getSkillsLabURL, getSkillsLabGateURL } from '../utils/page.js';
...
test.describe('MCPs', () => {
  ...
  test('Creating new MCP after editing one clears the editing state', async ({ page }) => {
    ...
    await page.getByRole('button', { name: '+ Register MCP' }).click();
    await expect(page.locator('.editor-title')).toContainText('Register MCP Server');
    await expect(page.getByLabel('MCP server id')).not.toHaveAttribute('readonly');
  });
...
test.describe('Accessibility — keyboard primary actions', () => {
  ...
  test('MCP menu supports keyboard navigation and Enter activation', async ({ page }) => {
    ...
    // nx-popover doesn't implement ArrowDown menu navigation; click the item.
    await editItem.click();
    await expect(page.getByLabel('MCP server id')).toBeVisible({ timeout: 5000 });
    await expect(page.getByLabel('MCP server id')).toHaveValue('pw-kbd-menu-mcp');
  });
...
test.describe('Performance UX', () => {
  test('Restores cached snapshot immediately and refreshes in background', async ({ page }) => {
    ...
    const slowConfigHandler = async (route) => {
    ...
    await page.route(`**/config/${TEST_ORG}/${TEST_SITE}/`, slowConfigHandler);
  ...
  test('Loads MCP tools lazily on MCP tab activation', async ({ page }) => {
    ...
    // Verify _refreshingCount is positive while the fetch is in-flight.
    const refreshing = await page.evaluate(() => {
      const el = document.querySelector('nx-skills-editor');
      return el?._refreshingCount ?? -1;
    });
...
test.describe('Cross-tab regression', () => {
  test('Skill editor opens after switching from MCPs tab', async ({ page }) => {
    ...
    await expect(page.getByLabel('MCP server id')).not.toBeVisible();
